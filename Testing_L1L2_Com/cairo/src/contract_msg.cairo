...
trait IContractL1<T> {
    ...
    //Sends a mesage to L1 contract that would be sent in the case of a requestDeposit
    fn send_message_deposit(ref self: T, to_address: EthAddress, receiver: felt252, asset: felt252, shares: felt252, exchange_rate: felt252);
...
mod contract_msg {
    ...
    impl ContractL1Impl of IContractL1<ContractState> {
        ...
        fn send_message_deposit(ref self: ContractState, to_address: EthAddress, receiver: felt252, asset: felt252, shares: felt252, exchange_rate: felt252) {
            let mut message_payload: Array<felt252> = ArrayTrait::new();
            message_payload.append(receiver);
            message_payload.append(asset);
            message_payload.append(shares);
            message_payload.append(exchange_rate);
            send_message_to_l1_syscall(to_address.into(), message_payload.span()).unwrap_syscall();
        }
    }
}
